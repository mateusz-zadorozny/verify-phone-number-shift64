...
In this repository that release process is `semantic-release`, driven from `.releaserc.json` and `.github/workflows/release.yml` on every push to `master`. It derives the version from the merged commit messages, so the squash-merge subject is what ships: `feat:` bumps the minor, `fix:`/`perf:` the patch, everything else releases nothing. `scripts/update-version.sh` propagates the new version into the plugin header, `readme.txt`, and `SHIFT64_PHONE_VALIDATION_VERSION`.
...
In this repository, the surfaces that most often mean `risk-high` are the checkout validation path (a false rejection blocks a real order) and the `shift64_phone_validation_*` filters and options listed in `BACKWARD_COMPATIBILITY.md`.
...
- Apply `needs-qa` to UI changes, new features, and other user-facing behavior that needs manual exercise. In this repository that means anything reaching the checkout — classic or block — or the settings screen.
...
QA for this plugin runs against a local WooCommerce store, not a scratch harness: place an order through both the classic (shortcode) and block (Store API) checkout, with a valid number, an invalid one, and an empty optional shipping phone.
...
One repository-specific constraint sits on top: the PR title is validated by `.github/workflows/pr-lint.yml` against the Conventional Commits types, and because merges are squashed that title becomes the release-driving commit subject. An agent that opens a PR here writes the title accordingly.
...
The `composer install` step is part of the gate on purpose: `vendor/` is git-ignored, and the agent skills work in freshly created worktrees where it does not exist yet. Without it the gate fails as "phpcs: command not found" rather than as a real code problem. It mirrors the same step in `.github/workflows/code-quality.yml`.
